...
public class RateLimitUtil {
    ...
    public Boolean rateLimit(HttpServletRequest request, Object handler) {
        if (handler instanceof HandlerMethod) {
            ...
            //验证是否限流
            if (redisTemplate.hasKey(RedisKeyConstant.LIMITTING_TOTAL) || redisTemplate.hasKey(limittingRateIpKey)) {
                return true;
            }
            Integer totalRate = (Integer) redisTemplate.opsForValue().get(RedisKeyConstant.RATE_LIMIT_TOTAL);
            if (totalRate != null && totalRate > RateLimitConstant.TOTAL_LIMIT_NUM) {
                //五秒内超过200请求 限流5分钟
                redisTemplate.opsForValue().set(RedisKeyConstant.LIMITTING_TOTAL, null
                        , RateLimitConstant.TOTAL_LIMIT_MINUTE, TimeUnit.MINUTES);
                redisTemplate.delete(RedisKeyConstant.RATE_LIMIT_TOTAL);
                return true;
            } else if (totalRate == null) {
                //第一次请求统计
                redisTemplate.opsForValue().set(RedisKeyConstant.RATE_LIMIT_TOTAL, 1, 5, TimeUnit.SECONDS);
            } else {
                //请求统计自增
            ...
            if (ipRate != null && ipRate > RateLimitConstant.IP_LIMIT_NUM) {
                ...
                if ((currentTimeMillis - lastRateTime) < TimeUnit.MINUTES.toMillis(1)) {
                    redisTemplate.opsForValue().set(limittingRateIpKey, null
                            , RateLimitConstant.IP_LIMIT_MINUTE, TimeUnit.MINUTES);
                    return true;
                }
            }
        }
